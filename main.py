# -*- coding: utf-8 -*-
"""Inference pipeline for Stanford RNA 3D Folding Part 2."""
...
import pandas as pd
import torch
from Bio.Align import PairwiseAligner  # TBM：查询与训练序列的全局比对
...
DATA_BASE              = "/root/autodl-tmp/RNA3D_Project/data/stanford-rna-3d-folding-2"
...
DEFAULT_OUTPUT         = "/root/autodl-tmp/RNA3D_Project/outputs/submission.csv"

DEFAULT_CODE_DIR       = "/root/autodl-tmp/RNA3D_Project/Protenix-v1"
DEFAULT_ROOT_DIR       = DEFAULT_CODE_DIR

MODEL_NAME    = "protenix_base_20250630_v1.0.0"
N_SAMPLE      = 10  # 提交列数 x_1..x_10；评测可能对多样本聚合，与 [2][3] 共同影响分数
SEED          = 42    # 全局种子；改此值可消融“随机性”本身（见上文随机因素 [1][3]）
...
def build_configs(input_json_path: str, dump_dir: str, model_name: str):
    ...
    arg_str = " ".join([
        ...
        f"--sample_diffusion.N_sample {MODEL_N_SAMPLE}",  # Protenix 每 target 采样条数；与随机因素 [3] 相关
        f"--seeds {SEED}",  # 传入推理配置；与 SEED / seed_everything 应对齐以便复现
    ])
    return parse_configs(configs=base, arg_str=arg_str, fill_required_with_null=True)


def get_c1_mask(data: dict, atom_array) -> torch.Tensor:
    """InferenceRunner.predict 返回后，从 data/atom_array 解析 C1' 掩码（与上一 cell 逻辑类似，字段略多）。"""
    if atom_array is not None:
        try:
            if hasattr(atom_array, "centre_atom_mask"):
                m = atom_array.centre_atom_mask == 1
...
def coords_to_rows(target_id: str, seq: str, coords: np.ndarray) -> list:
    """将 (N_SAMPLE, seq_len, 3) 展平为竞赛要求的 CSV 行：每残基一行，列 x_1..x_10, y_*, z_*。"""
    rows = []
...
def split_into_chunks(seq_len: int, max_len: int, overlap: int) -> list:
    """超长序列切分为重叠窗口，满足 Protenix token 上限 MAX_SEQ_LEN；overlap 用于拼接时对齐。"""
    if seq_len <= max_len:
        return [(0, seq_len)]
    chunks = []
...
def kabsch_align(P: np.ndarray, Q: np.ndarray):
    """Kabsch：用重叠残基对齐相邻 chunk 的坐标系，便于 stitch_chunk_coords 融合。"""
...
def stitch_chunk_coords(chunk_coords_list: list,
    ...
    """
    多 chunk Protenix 输出拼回全长：相邻块在重叠区做 Kabsch，再按线性权重融合，
    避免接缝处断裂（长序列如 9MME、9ZCC 会走此路径）。
    """
...
def tbm_phase(test_df, train_seqs_df, train_coords_dict, segments_map):
    ...
    对每个测试 target：按相似度取模板 → adapt 坐标 → 多样性变换 → adaptive_rna_constraints。
    若模板不足 N_SAMPLE，则记录 protenix_queue，由 Phase2 用 Protenix 补全。
...
def main() -> None:
    ...
    if protenix_queue and USE_PROTENIX:
        ...
        work_dir = Path("/root/autodl-tmp/RNA3D_Project/workdir")
        ...
        for i in tqdm(range(len(dataset)), desc="Protenix Inference"):
            ...
            try:
                ...
                new_cfg.sample_diffusion.N_sample = n_needed  # 本 target 需补几条就与 TBM 对齐
                runner.update_model_configs(new_cfg)
                
                pred = runner.predict(data)  # 扩散采样，随机因素 [3]
    ...
    for _, row in test_df.iterrows():
        ...
        while len(combined) < N_SAMPLE:
            seed_val = row.name * 1000000 + len(combined) * 1000  # 随机因素 [4]
...
submission_path = DEFAULT_OUTPUT
